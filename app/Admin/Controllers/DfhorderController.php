...
class DfhorderController extends Controller
{
    ...
    protected function grid()
    {
        ...
        $grid->id('Id');
        $grid->sn('交易号')->expand(function ($model) {

            $comments = $model->items()->get()->map(function ($comment) {
                return [
                    'id' => $comment->id,
                    'goods' => Product::find($comment->goods_id)->title,
                    'pic' => '<img src="'.env('APP_URL').'/uploads/'.Product::find($comment->goods_id)->pics[0].' " width=100>',
                    'num' => $comment->num,
                ];
            });

            return new Table(['ID','产品','图片','数量'], $comments->toArray());
        });
        ...
        $grid->column('fh','操作')->display(function ($v){
            if ($this->status ==1) {
                return '<a href="/'.env('ADMIN_ROUTE_PREFIX').'/ps?id='.$this->id.'"><button>确认配送</button></a>';
            }
        });
