...
class ZdshOrder implements ShouldQueue
{
    ...
    public function handle()
    {
        // 判断对应的订单是否已经被支付
        // 如果已经支付则不需要关闭订单，直接退出
        if ($this->order->is_ys || $this->order->status == 4) {
            return;
        }
        // 通过事务执行 sql
        \DB::transaction(function() {
           ...
           // Msg::sendmsg(3, $this->order->tel, $str);
            ...
            if ($leader) {
                ...
                if ($xsuser) {
                    $xsuser->money +=  $this->order['price'] * XsUser::whereUId($xsuser->id)->first()['tc'];
                    $xsuser->zmoney +=  $this->order['price'] * XsUser::whereUId($xsuser->id)->first()['tc'];
                    $xsuser->save();
                    SyMsg::create([
                        'u_id' => $xsuser->id,
                        'msg' => '社区代理' . $leader->nickname . '用户' . User::find($this->order->u_id)->nickname . '购买产品',
                        'time' => date('Y-m-d H:i:s', time()),
                        'sy' =>  $this->order['price'] * XsUser::whereUId($xsuser->id)->first()['tc']
                    ]);
                }
            }
            ...
            $psorder = DB::table('log_ps')->where('id',PsList::whereOrderId($this->order->id)->first()->log_ps_id)->sharedLock()->first();

            if ($psorder->is_js) {
                return;
            }
            DB::table('log_ps')->where('id',PsList::whereOrderId($this->order->id)->first()->log_ps_id)->update([
                'is_js' => 1
            ]);
            if ($thd) {
                ...
                SyMsg::create([
                    'u_id' => $thd->id,
                    'msg' => '提货点收益-用户' . User::find($this->order->u_id)->nickname . '购买产品',
                    'time' => date('Y-m-d H:i:s', time()),
                    'sy' => $psorder->price
                ]);
            }
