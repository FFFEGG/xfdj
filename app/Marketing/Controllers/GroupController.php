...
use App\Marketing\Extensions\GroupExporter;
use App\Marketing\Extensions\PostsExporter;
use App\Region;
use App\User;
...
class GroupController extends Controller
{
    ...
    protected function grid()
    {
        ...
        $grid->region_id('区域')->editable('select', $arr);
        $grid->xqname('小区名称');
        $grid->address('社区地址')->width(200);
        $grid->name('负责人姓名');
        $grid->tel('负责人电话');
        ...
        $grid->disableExport(false);
        $grid->exporter(new GroupExporter());
    ...
    protected function form()
    {
        ...
        $form->select('region_id','区域')->options(function (){
            $data = Region::get();
            foreach ($data as $v) {
                $arr[$v->id] = $v->name;
            }
